...
def get_wiki_context(wiki_html):
    soup = BeautifulSoup(wiki_html, 'html.parser')
    structured_context = get_structure_context(soup, wiki_html)
    return structured_context


def get_structure_context(soup, html):
    ## INIT
    output = []
    last_position = 0

    a_tag = soup.find('a', text='검색하러 가기')
    last_position = get_tag_position(a_tag, html, last_position)
    div_soup = a_tag.find_next('div').find('div')
    assert div_soup is not None, '개똥 된거임............'
    last_position = get_tag_position(div_soup, html, last_position)

    last_position = get_tag_position(div_soup, html, last_position)
    for child in div_soup.children:
        last_position, temp_output = get_structure_tag_info(child, html, last_position)
    ...
    if target_list is not None and len(target_list) > 0:
        if tag_name in ['table', 'ul', 'dl', 'tr']:
            ...
            for child in soup.children:
                last_position, child_output = get_structure_tag_info(child, html, last_position)
        ...
        if soup.name in ['p', 'h2', 'h3', 'dt', 'dd', 'li', 'td', 'th']:
            ...
            last_position, position_ids = get_text_positions(text, html, last_position)
...
def get_text_positions(text, html, last_position):
    position_ids = []
    for substr in text:
        last_position = get_str_position(substr, html, last_position)
        position_ids.append(last_position)
    return last_position, position_ids


def get_str_position(substr, html, last_position):
...
def get_tag_position(soup, html, last_position):
    tag_name = soup.name
    tag_html = str(soup)
    ...
    last_position = get_str_position("<" + tag_name, html, last_position)
    return last_position
